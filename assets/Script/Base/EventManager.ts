...
export default class EventManager extends Singleton {
    ...
    off(event: string, func: Function, ctx?: unknown) {
        if (this.eventDic.has(event)) {
            let index: number;
            if (ctx) {
                index = this.eventDic.get(event).findIndex(i => func === i.func && ctx === i.ctx);
            } else {
                index = this.eventDic.get(event).findIndex(i => func === i.func);
            }
    ...
    emitOnce(event: string, ...params: unknown[]) {
        if (this.eventDic.has(event)) {
            this.eventDic.get(event).forEach(({ func, ctx }) => {
                ctx ? func.apply(ctx, params) : func(...params);
                this.off(event, func);
