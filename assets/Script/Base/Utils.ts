...
export function formatTimeInterval(seconds: number, alwaysShowMinutes: boolean = false, alwaysShowHours: boolean = false) {
...
export function truncateString(str: string, length = 8): string {
    // 判断字符串长度是否超过8位
    if (str.length > length) {
        return str.slice(0, length) + '...';  // 截取前8个字符并加上"..."
    }
